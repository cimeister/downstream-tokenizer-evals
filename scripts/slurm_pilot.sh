...
# ============================================================
# Pilot: single node (4 GPUs)
# Usage: sbatch scripts/slurm_pilot.sh <config> <tokenizer> <data_dir> [run_name]
# Example: sbatch scripts/slurm_pilot.sh configs/pilot_128k.yaml /path/to/tok /path/to/data
# ============================================================

set -euo pipefail

CONFIG="${1:?Usage: sbatch slurm_pilot.sh <config.yaml> <tokenizer> <data_dir> [run_name]}"
TOKENIZER="${2:?Missing tokenizer path}"
DATA_DIR="${3:?Missing data directory}"
RUN_NAME="${4:-pilot-$(basename ${TOKENIZER})}"
...
echo "=== Pilot Training ==="
echo "Job: ${SLURM_JOB_ID} | Nodes: ${SLURM_JOB_NUM_NODES} | GPUs: 4"
echo "Config: ${CONFIG}"
